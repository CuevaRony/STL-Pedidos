...
void Lista::actualizalista(vector <NProductos> vprod){
    int codprod,prio,cant,min;
    Nodo *rec,*ant=NULL,*mov;
    
    rec = lini;
    while(rec)
    {   codprod=rec->ped.GetCodigo();
        for(vector<NProductos>::iterator it=vprod.begin();
            it!=vprod.end(); it++){
            if(it->getCodprod()==codprod){
                prio = it->getprioridad();
        ...
        if(prio){
           ...
           // if(rec->ped.GetCantidad()>cant )   
           //     rec->ped.setDespacho(1);
            rec = ant;
        }  
        //else{
        //    if(rec->ped.GetCantidad()>cant )   
